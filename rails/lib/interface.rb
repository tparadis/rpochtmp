module Interface
	...
	def Interface.ouvertAuj?(id)
		...
		#On remet les horaires de la semaine sous forme de tableau pour les renvoyer
		semaine = {}
		semaineT = []
		indice = 0
		numJour = 0
	...
	def Interface.getCommerceByID(lid)
		id = Commerce.where("id = ?",lid).first.id
		com = Commerce.where(:id => lid).first.as_json
		com.merge!({:note => (Note.where("commerce = ?", id).length > 0 ? (Note.where("commerce = ?",id).sum(:note) / Note.where("commerce = ?",id).length).to_s + "/5" : "(pas de note)" )})
		
		#Si c'est le noimage.jpg, alors on remplace par une image
		#de catégorie générique
		base_url = "/srv/http/static/images/categories/"
		if com["image"]["url"].include? "/noimage.jpg"
			begin
				new_img = base_url + Sscategory.where("id = ?",com["tag0"]).first.catparent.to_s + ".jpg"
				if !File.file?(new_img)
					new_img = "/srv/http/static/images/noimage.jpg"
				end
				com["image"]["url"] = new_img
			rescue
